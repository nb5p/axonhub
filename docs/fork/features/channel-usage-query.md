...
- 渠道列表的单行刷新按钮只在脚本返回 `text` 或 `balance` 时显示；仅返回 tags／进度窗口的渠道统一由右上角提供商配额刷新，避免出现无意义的行内按钮。
- `progress.windows` 可包含任意数量的独立窗口，五个或更多同样有效。窗口可只提供 `id`，也可不提供重置时间；缺少可计算的百分比时不会渲染填充条。配额卡的窗口标题行采用“`id（重置于…）` … `剩余 X%`”，用量条独占下一行。
...
| 2026-08-22 | 本地窗口排版修正 | `f9bca34caca2` | original：将脚本窗口的重置倒计时移入标题行左侧，并在右侧统一展示剩余百分比。 |
| 2026-08-22 | 本地交互修正 | `e61ef8b6e70e` | original：仅 tags／进度窗口的用量查询渠道不再显示列表行内刷新按钮。 |
