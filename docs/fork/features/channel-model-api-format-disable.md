...
例如 OpenCode Go 的 `glm-5.2`、`glm-5.3` 可能接受 OpenAI Chat Completions，却在 OpenAI Responses 请求中错误校验 Codex 原生工具对象。对该模型禁用 `openai/responses` 后，普通 Responses 入站请求优先在同一渠道改走 `openai/chat_completions`；只有没有可用端点时才交给既有 fallback 选择其他渠道。
...
- 本地原创功能，不移植外部代码。
- 2026-08-23 对比 `upstream/unstable@37e54737c0e147831c6a0b529b79dda1961c3801` 的可见接口、渠道设置、端点选择和测试链路，未发现按“渠道 × 模型 × 下游 API 格式”禁用的同类实现。

## 本地实现

- `channels.settings.disabledModelApiFormats` 是可选数组。每项包含 `model` 与 `apiFormats`；保存时去除空白、合并同名模型、去重格式，并拒绝空模型或无有效格式的项。
...
上游基线仅根据渠道端点整体选择 API 格式，不具备模型级例外。本实现复用现有端点解析、格式转换、候选筛选和渠道测试链路，不引入第二套路由器或 provider 特判。
...
兼容等级为 `additive`。不修改 Ent Schema、表、列、索引或迁移；仅在既有 `channels.settings` JSON 中添加可选 `disabledModelApiFormats` 字段。缺失字段等价于没有禁用规则，旧数据无需回填。

旧版本可以读取包含未知 JSON 字段的渠道记录，但若用旧版本保存已配置此规则的渠道设置，旧 GraphQL 输入无法带回该字段，可能覆盖规则。部署前应按蓝绿流程创建 SQLite 一致性备份；回退到旧版本前避免编辑相关渠道，或恢复部署前快照。
