...
local:
  ...
  commits:
    ...
    - f6af276d886f3c8c9dafd3c89c25b191f85de5b6
    - 6ded56c8e55228c874303b16439eb9c73a96eda4
    - ce97d187d791da3e0b612e7456563939c9e70137
  modules:
...
- 分页和无限滚动模式都允许自动刷新。分页模式仍仅在第一页自动刷新；无限滚动模式刷新当前已加载的查询页，并保留滚动位置和已加载范围。
...
- 关闭功能不会建立新的后台任务。渠道原有 5 秒状态轮询仍然存在；绝大多数少于 1000 个渠道的部署仍为单次请求，超过 1000 个渠道时每轮会按页顺序请求。请求日志开启自动刷新后每 10 秒刷新一次；无限滚动已加载多页时会重取这些页面，请求量随已加载页数增长。
...
- `node --test src/features/requests/request-auto-refresh.test.mjs`：通过，确认关闭分页后自动刷新仍可开启、定时器继续刷新无限查询。
...
| 2026-08-20 | 本地修复 | `ce97d187d791da3e0b612e7456563939c9e70137` | fix：解除无限滚动与自动刷新的旧互斥限制，保留非分页模式的自动刷新能力。 |
