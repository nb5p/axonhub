...
local:
  ...
  commits:
    ...
    - faa9a8c462605c8f9d66e9adbe50e063caf6f4f0
  modules:
...
- 渠道状态列虽然固定为不可隐藏，但仍显示在“列设置”中参与拖拽排序；其可见性复选框保持选中并禁用。
- 渠道标签单元格使用 `ResizeObserver` 计算当前可容纳的标签数量；剩余标签折叠为 `+N`，悬停或触摸后仍可查看。
- 标签列默认显示并可在列设置中显隐；旧版保存的默认隐藏值会一次性迁移为显示，防止标签完全没有恢复入口。
...
| 2026-08-14 | 用户反馈 | `faa9a8c462605c8f9d66e9adbe50e063caf6f4f0` | 渠道状态列进入列设置参与拖拽排序，同时继续禁止隐藏。 |
| 2026-08-22 | 用户反馈 | `6de0fbbe6edf` | 恢复渠道标签列默认可见与可配置状态，并兼容迁移旧浏览器偏好。 |
