...
local:
  ...
  commits:
    ...
    - d54b3fdf759465b66a14d83c15c0cfd0ba7ef9ab
  modules:
...
本地原创实现，覆盖 Codex 和 OpenCode Go 的配额展示，以及提供商配额弹层内的脚本进度窗口和标签。其他提供商的配额逻辑保持原有显示逻辑。

此前当日用量标签的视觉和数据语义参考 Sub2API 的 `AccountUsageCell.vue`。该展示现已从提供商配额弹层移除；参考仓库采用 LGPL-3.0，本实现没有复制其 Vue 代码、样式文本或数据访问代码。
...
- 提供商配额的脚本渠道行只展示 `tags` 和进度窗口，标签与电池图标左对齐；不显示脚本文本或脚本余额。Codex OAuth 例外：显示今日请求数、Token 与 A$ 实际成本三枚统计标签，并与脚本 tags 共用同一行。
- 每个脚本窗口的标题行左侧显示“窗口 ID（重置于…）”，右侧显示“剩余 X%”；用量进度条独占下一行，避免重置倒计时挤在进度条下方。
- 脚本查询的余额仅在渠道列表和脚本测试结果中固定显示两位小数；OpenCode Go 以 USD 余额而非文本加入这一统一展示路径。
- 脚本余额的通用文案为“剩余：数值 单位”，例如 `剩余：12.00 USD`；不调用浏览器的货币符号本地化。`A$` 继续显示为 `A$12.00`。
- 提供商配额的前端查询不再读取当天 `usage_logs` 聚合；保留原有配额状态的 60 秒刷新周期，未改动 Ent Schema 或数据库结构。
...
| 2026-08-22 | 本地展示边界修正 | `3aa763c106b2` | 配额卡只保留脚本 tags 和进度窗口，渠道列表只保留 text 与余额，并移除未使用的当天用量聚合查询。 |
| 2026-08-22 | 本地窗口排版修正 | `f9bca34caca2` | 将重置倒计时合入窗口标题左侧，并在右侧显示剩余百分比。 |
| 2026-08-22 | 本地回归修复 | `d54b3fdf759465b66a14d83c15c0cfd0ba7ef9ab` | 恢复被“隐藏脚本余额”变更误删的 Codex OAuth 今日请求数、Token、A$ 三枚统计标签；A$ 固定显示为两位小数。 |
