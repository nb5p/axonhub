...
func TestNormalizeSub2APICredentials(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "Grok account export retains OAuth client for subscription channel",
			provider:         "grok",
			raw:              `{"platform":"grok","type":"oauth","credentials":{"access_token":"grok-access","refresh_token":"grok-refresh","client_id":"grok-cli-client","base_url":"http://untrusted.example/v1","expires_at":"1787306400000"}}`,
...
func TestNormalizeSub2APICredentials_RejectsMismatchedPlatform(t *testing.T) {
	now := time.Date(2026, time.August, 21, 10, 0, 0, 0, time.UTC)
