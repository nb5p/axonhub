...
func IsModelAPIFormatDisabled(
	...
	for _, disabled := range settings.DisabledModelAPIFormats {
		...
		if !matchesDisabledRequestClient(disabled.Clients, requestClient) {
			continue
		}
...
func FilterEndpointsForModel(
	...
	for _, endpoint := range endpoints {
		if IsModelAPIFormatDisabled(settings, entry, endpoint.APIFormat, requestClient) {
			continue
		}
...
func matchesDisabledRequestClient(clients []string, requestClient llm.RequestClient) bool {
	if len(clients) == 0 {
		return true
	}

	requestClient = llm.NormalizeRequestClient(requestClient)
	for _, client := range clients {
		if client == string(requestClient) {
			return true
		}
	}
